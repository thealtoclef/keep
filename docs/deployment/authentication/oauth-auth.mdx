...
-   **`identitymanager/identity_managers/oauth/oauth_authverifier.py`**: This is the core of the backend authentication logic.
    ...
    -   **JWT Verification**: The token is verified using `NEXTAUTH_SECRET` to ensure authenticity and extract user information.
    ...
    -   **Short-lived Tokens**: Tokens are valid for 1 minute, which is a security best practice for tokens that are used immediately for verification.
...
-   **`NEXTAUTH_SECRET`**: This is required for signing and verifying the JWT.
-   **Provider Credentials**: Add the `CLIENT_ID` and `CLIENT_SECRET` for your new provider.

## Security Features

-   **JWT-based Authentication:** Tokens are signed with `NEXTAUTH_SECRET` using the HS256 algorithm.
-   **Short-lived Tokens:** 1-minute token expiration for security best practices.
